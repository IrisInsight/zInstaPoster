...
A blunt word list is the other way a rule goes wrong, and it goes wrong by
blocking copy that is fine. `banned_phrases` takes three lists, so a rule can
say which kind of wrong a word is:

| param | matches |
| --- | --- |
| `phrases` | wherever the phrase appears — "clinically proven" is never sayable |
| `claim_verbs` | only in claim position, a verb performed on something: "reverses hair loss" is a claim, "there is no cure for it" is not |
| `allow_terms` | nothing — a term of art that contains a banned word, exempted by name |

That is what lets the outcome-claims rule keep banning "reverse" while the
practice names reverse T3, the thyroid lab value, on a slide. Adding a clinical
term the checker should not read as a claim is one line of tenant config, not a
code change — and never widening the word itself.
...
and a script sets it to the largest size that still fits its column — which is
what keeps a short headline filling the frame instead of floating in the middle
of it. The fit runs after `document.fonts.ready`, because a size measured
against the fallback serif is a size chosen for a font that is not on the slide;
the renderer waits for the fit to finish before the screenshot is taken.
